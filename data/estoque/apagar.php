...
/*
O estoque irá enviar os dados no formato json. Porém nós precisamos dos dados
 no formato php para deletar em banco de dados. Para realizar essa conversão
 iremos usar o comando json_decode. Assim que o estoque enviar os dados, estes são
...
if(!empty($data->id)){
    ...
    if($estoque->apagar()){
        ...
        header("HTTP/1.0 400");
        echo json_encode(array("mensagem"=>"Não foi possível apagar o estoque."));
    }
}
else{
    header("HTTP/1.0 400");
    echo json_encode(array("mensagem"=>"Você precisa passar todos os dados para apagar o estoque"));
}
?>