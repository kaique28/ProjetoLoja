...
O estoque irá enviar os dados no formato json. Porém nós precisamos dos dados
 ...
 iremos usar o banco json_decode. Assim o estoque enviar os dados, estes são
...
if(!empty($data->id_produto) && !empty($data->quantidade) && !empty($data->id)){
    ...
    if($estoque->atualizar()){
        header("HTTP/1.0 200");
        echo json_encode(array("mensagem"=>"estoque atualizado com sucesso!"));
    }
    else{
        header("HTTP/1.0 400");
